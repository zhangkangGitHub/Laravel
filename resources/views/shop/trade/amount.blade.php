...
<div class="margin clearfix">
 <div class="amounts_style">
   <div class="transaction_Money clearfix">
      <div class="Money"><span >成交总额：{{$total}}元</span><p>最新统计时间:{{$time}}</p></div>
       <div class="Money"><span ><em>￥</em>{{$totals}}元</span><p>当天成交额</p></div>
       <div class="l_f Statistics_btn"> 
     </div>
    </div>
    <div class="border clearfix">
      <span class="l_f">
      <a href="/shop/amount" class="btn btn-info">全部订单</a>
      <a href="/shop/amount?all=1" class="btn btn-danger">当天订单</a>
        <a href="/shop/amount?all=2" class="btn btn-danger">当月订单</a>
       </span>
       <span class="r_f">共：<b>{{$num}}</b>笔</span>
     </div>
   <div class="Record_list">
    <table class="table table-striped table-bordered table-hover" id="sample-table">
       <thead>
        ...
        <tbody> 
        <div style="display:none">{{$tm = date('d',time())}}</div>
        <div style="display:none">{{$tms = date('m',time())}}</div>
